...
X = data.frame(scale(dyslipidemia_no_na[, 2:(df_len-2)]))
...
for (i in 1:length(probs_tuning) ){
    test_predictions_logit <- predict(logistic_regression, newdata=test, type='response')
    test_predictions_logit <- ifelse(test_predictions_logit > probs_tuning[i], 1, 0)
    
    table_results <- table(test$CVDMEDST, test_predictions_logit)
    
    accuracy[i] <- (table_results[1] + table_results[4])/sum(table_results)
    fpr[i] <- table_results[3]/(table_results[1] + table_results[3])
    fnr[i] <- table_results[4]/(table_results[2] + table_results[4])
    f1[i] <- F1_Score(test$CVDMEDST, test_predictions_logit, positive=1)
}
...
CrossTable(test$CVDMEDST, test_predictions_logit)

misClasificError <- mean(test_predictions_logit != test$CVDMEDST)
print(paste('Accuracy: ',1-misClasificError))

print(paste("F1 score: ", F1_Score(test$CVDMEDST, test_predictions_logit, positive=1)))
...
for (i in 1:length(probs_tuning) ){
    test_tuning_logit <- predict(logistic_regression, newdata=test, type='response')
    test_tuning_logit <- ifelse(test_tuning_logit > probs_tuning[i], 1, 0)
    
    table_results <- table(test$CVDMEDST, test_tuning_logit)
...
ggplot(tuning_results, aes(x=fnr, y=fpr)) + geom_point(colour="red", size=2.5)
