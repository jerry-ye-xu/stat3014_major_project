...
density_alpha_linoleic <- ggplot(cholesterol_no_na, aes(x = cholesterol_no_na$ALAPER1, fill = LDLNTR)) +
  geom_density(alpha = 0.5) +
    scale_fill_discrete(labels=c("0", "1")) + guides(fill=guide_legend(title=NULL)) +
...
density_systol <- ggplot(df, aes(x = cholesterol_no_na$SYSTOL, fill = LDLNTR)) +
  geom_density(alpha = 0.5) +
    scale_fill_discrete(labels=c("0", "1")) + guides(fill=guide_legend(title=NULL)) +
