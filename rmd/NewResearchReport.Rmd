...
dyslip_test_predictions_logit <- ifelse(test_predictions_logit > prob_threshold, 1, 0)
...
The random forest model results in a high FPR, and thus its performance may be less desirable if your aim is to reduce Type I errors. 
...
Our results were interesting but nowhere near as consistent with the nutritional literature as we would have liked. The most significant result was that age and BMI were extremely important in affecting both diabetes and dyslipidemia. This is reflected in both the logistic regression output and the importance measure from the random forest. This is not unexpected, but it is reassuring to see this reflected in the output.  

This is unfortunately where the nice results end. For diabetes, our logistic regression model identified a number of significant factors. The factors that contributed to having diabetes (read from the parameter estimates) were higher BMI, being older, and greater alpha linoleic acid. The factors that help reduce the chances of diabetes are polyunsaturated fat, blood pressure, niacin, fibre, alcohol and free sugar. These is interesting and does not entirely agree with the literature (alcohol prevents diabetes?), but the effect of free sugar is good. Although sugar as a whole contributes to having diabetes, replacing added sugars with free sugars (the better alternative) does help to reduce the chances, as we have observed. 
...
Our predictions using both the logistic regression and the random forest were encouraging but not ideal. Using the logistic regression, we could achieve a small false positive and false negative rate. This outperformed the random forest, which struggled to deal with the class imbalance. 

For dyslipidemia, the logistic regression again identified BMI and age as factors associated with dyslipidemia. Trans fat and blood pressure were on the brink of significance in contributing to dyslipidemia, while again alcohol was apparently associated with reducing chances. 
The most important factors identified by the random forest were 

There are a number of reasons for why the results may have turned out this way. Firstly, as seen in our exploratory analysis there didn’t appear to be any significant differences in the micronutrient intake of those with and without these NCD’s. Therefore, of course these variables would not flag as significant because in our real world sample, there was actually no difference.

Alcohol registering as a significant variable in our analysis was interesting, but is probably due to its skewed distribution. There is a high number of sampling zeros, due to the fact that many adults do not drink. Alcohol, along with many of the variables, also has a high number of outliers that we did not address which may be causing the strange results.

Additionally, as stated earlier the class imbalance makes classification hard. Particularly in diabetes, there is such a low proportion of people with diabetes relative to the sample size that it is hard to find exactly what it is that distinguishes that class from another. With more data points or a more balanced design, it may be possible to gain greater insight. 

# Conclusion 

In conclusion, further analysis should be undertaken to critically evaluate the role of these fats and carbohydrates on diabetes and dyslipidemia. As expected, age and BMI were significant factors but we did not strongly observe the effects of certain micronutrients we expected from the literature. We did see positive results for sugars, in that natural sugars contributed to the non-prevalence of disease (as it is the healthier option to added sugars). And finally, it is possible to predict if a person has type II diabetes or dyslipidemia from their nutritional habits.
