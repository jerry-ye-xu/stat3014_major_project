...
We check the null accuracy, which is 63%.
...
density_alpha_linoleic <- ggplot(df, aes(x = df$ALAPER1, fill = CHOLNTR)) +
  geom_density(alpha = 0.5) +
    scale_fill_discrete(labels=c("0", "1")) + guides(fill=guide_legend(title=NULL)) +
...
density_systol <- ggplot(df, aes(x = df$SYSTOL, fill = CHOLNTR)) +
  geom_density(alpha = 0.5) +
    scale_fill_discrete(labels=c("0", "1")) + guides(fill=guide_legend(title=NULL)) +
