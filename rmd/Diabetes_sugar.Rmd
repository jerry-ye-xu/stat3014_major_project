...
Much like its fats-analysis counterpart, the overwhelming class imbalance requires upsampling and techniques again. Upsampling the data has resulted in many of the predictor variables becoming significant, which should be taken with a grain of salt given that the samples are replicated with slight modifications (as in SMOTE).

The only substantial improvement with prediction occurred when using the SMOTE algorithm for upsampling. We were able to achieve a $\approx$20% FNR for a 40% increase in FPR, which is reasonable given the imbalanced test set used to evaluate the algorithm. Penalised-SVM could not achieve the same FNR despite tuning parameters, whilst Random Forests were substantial worst with a $\approx$70% FNR, despite keeping the FPR under 10%
...
With a null accuracy of $\approx$94%, it is going to be difficult to use the standard classification algorithms to acheive any improvements in prediction accuracy.
...
density_exercise <- ggplot(df, aes(x = df$EXLWTBC, fill = df$DIABBC)) +
  geom_density(alpha = 0.5) +
    scale_fill_discrete(labels=c("0", "1")) + guides(fill=guide_legend(title=NULL)) +
...
```{r}
X <- as.matrix(diabetes_no_na[, 2:df_len])
y <- diabetes_no_na$DIABBC
...
Now we have a considerable reduction in null accuracy, with $\approx$64%.  
