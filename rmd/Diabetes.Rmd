...
In the oversampling methods, the original data was first partitioned (70/30) into a training set and a test set. This training set had the minority class upsampled such as that the class balanced was approximately 2/3 to 1/3. A random forest and logistic regression was fitted and used to predict for the test set, which contains data that has not been seen. The results improved slightly, but the Type II errors still constituted a majority of the errors. 

Lastly, the penalised-SVM with considerably increase in weights for the minority class acheived a more balanced approach, with 33% test samples resulting in a Type II error which is a significant reduction from the other 2 models used. However, this resulted in 25% Type I error, which arguably is a small trade-off if reducing Type II errors is the more important outcome. There was no upsampling done for this method, the original imbalanced dataset was used.  

The SMOTE method for random forests produced similar results, but the GLM models were able to produce significantly lower Type II error at the cost of an increase in Type I error. The trade-off is similar, but overall slightly better for the logistic model, with approximately 25% each for both types of errors. 

Further fine-tuning can perhaps increase the overall prediction accuracy.
...
density_bmi <- ggplot(df, aes(x = df$BMISC, fill = DIABBC)) +
...
It seems that SYSTOL, BMI and AGE are the 3 most distinguishable 1-dimensional variables by class. This makes sense, and allows for the models after some tuning to have some form of prediction. 
...
The random forest doesn't make it any better either. So now we need to try a different model, and then afterwards use upsampling. 

We skip the hyperparameter tuning step because random forests is certainly not the right model for this imbalanced dataset.
...
The previous methods did not work so well as the response variable classes are extremely imbalanced. The forest models simply predicted one class, whilst KNN and the like were unable to distinguish between the minority class. 
...
It seems that upsampling does not provide any meaningful improvements in prediction. There are still far too many Type II errors when using the original test set. 
...
# This will not work as many of the samples have been replicated. So of course, this will be much more accurate. 
    #print("Predict using the test set")
    #colnames(test)[1] <- "y"
    #diabetes_test <- predict(diabetes_train, data = test)
    #CrossTable(test$y, diabetes_test$predictions)
...
Using the penalised-SVM, we were able to reduce the number of false negatives by weighting the classes differently. The analysis suggests that there are different methods of finetuning the FPR and FNR depending on your problem and these models all have their strengths and weaknesses. 
...
The SMOTE method yielded a slightly better FNR compared to the same random forest models for basic upsampling. 

However, with GLMs, the results were much better initially. Hence, we need want to compare with a GLM model using SMOTE and see whether there is any improvement. 
...
The GLMs for SMOTE produce a lower Type II error but comparatively the Type I error is significantly higher compared to the other models. The trade-off seems to be consistent with the Random Forests model too. Overall, I think the SMOTE method is a better method for increasing minority class sample sizes for modelling. 

```{r}

probs_tuning <- seq(0.05, 0.95, by=0.01)
