...
diabetes_train <- ranger(
  ...
  data            = train,
  importance      = "impurity",
  num.trees       = hyper_grid_2[optimal_parameters_idx, "num.trees"],
...
(table_test[1] + table_test[4])/(sum(table_test))
...
#train_diabetes <- sample_frac(diabetes_no_na, 0.7, replace=FALSE)
#train_idx <-as.numeric(rownames(train))
...
TODO: Decide which training set to use?

Always: Use test_diabetes as that was the original split from diabetes_no_na
...
### 2) Penalised Support Vector Machines
...
print("Just checking that sample_frac functioned correctly")
table(train_diabetes$DIABBC)
table(test_diabetes$DIABBC)
table(diabetes_no_na$DIABBC)
...
table_test <- table(test$y, diabetes_test$predictions)
table_test_orig <- table(test_diabetes$DIABBC, diabetes_true_test$predictions)
...
We were able to improve the algorithm slightly, but significantly reduce the number of Type II errors. 